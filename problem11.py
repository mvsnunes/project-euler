...
product = []
# Loop through the grid with conditions to make the product
for row in range(20):
    for column in range(20):
        # Up
        if row > 3:
            product.append(grid[row][column]*grid[row-1][column]
        ...
        # Left
        if column > 3:
            product.append(grid[row][column]*grid[row][column-1]
        ...
        # Diagonally
        # Right-Up
        if column < 17 and row > 3:
            product.append(grid[row][column]*grid[row-1][column+1]
...
print("The greatest product of four adjacent numbers in the same direction is", max_product)
